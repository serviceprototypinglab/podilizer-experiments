...
public class LambdaFunction implements RequestStreamHandler {
    ...
    public void handleRequest(InputStream inputStream, OutputStream outputStream, Context context) throws IOException {
        ...
        try{
            prepare(inputType.getFileurl(), inputType.getFilename());
       } catch(Exception e ) {
           {
    OutputType outputType = new OutputType(this.stats, e, "Exception");
    objectMapper.writeValue(outputStream, outputType);;
}       };
        {
            OutputType outputType = new OutputType(this.stats, null, "null");
            objectMapper.writeValue(outputStream, outputType);;
        }
    }
