...
public class Transport {
    ...
    public void prepare(String fileurl, String filename) throws Exception {
        ...
        try {
            ...
            invokeRequest.setFunctionName(functionName);
            invokeRequest.setPayload(json);
            outputType = objectMapper.readValue(byteBufferToString(lambdaClient.invoke(invokeRequest).getPayload(), Charset.forName("UTF-8")), awsl.unpackaged.Transport.prepare2.OutputType.class);
        } catch (Exception e) {
        }
        ;
        this.stats = outputType.getStats();
